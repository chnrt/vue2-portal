...
export default {
  ...
  computed: {
    ...
    styleobj() {
      return {
        transform: `translate3D(${this.basicdata.poswidth},${this.basicdata.posheight},0)`,
      };
    },
    // pagenum滑动数
    pagenums() {
  ...
  swipeStart (e) {
    ...
    swipeEnd (e) {
        ...
        if (deltaTime > this.sliderinit.thresholdTime) {
            ...
            if ((deltaX > this.sliderinit.thresholdDistance)&&(Math.abs(deltaY)
  < this.sliderinit.thresholdDistance)) {
                //swipe right
                this.pre();
                return
            } else if ((-deltaX >
  this.sliderinit.thresholdDistance)&&(Math.abs(deltaY)
  < this.sliderinit.thresholdDistance)) {
                //swipe left
                this.next();
                return
            ...
            if ((deltaY >
  this.sliderinit.thresholdDistance)&&(Math.abs(deltaX)
  < this.sliderinit.thresholdDistance)) {
                //swipe right
                this.pre();
                return
            } else if ((-deltaY >
    this.sliderinit.thresholdDistance)&&(Math.abs(deltaX) < this.sliderinit.thresholdDistance)) {
                //swipe left
                this.next();
                return
            } else {
